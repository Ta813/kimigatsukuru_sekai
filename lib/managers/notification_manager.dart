...
class NotificationManager {
  ...
  bool _isRequestingPermission = false; // ★追加: 二重呼び出し防止フラグ
  ...
  Future<bool> requestPermission() async {
    if (_isRequestingPermission || PermissionManager.instance.isRequesting) {
      print(
        "⚠️ NotificationManager: Permission request already in progress. Ignoring.",
      );
      return false;
    }
    ...
    try {
      if (Platform.isIOS) {
        ...
        // Androidの場合: centralized PermissionManager を使用
        final status = await PermissionManager.instance.request(
      ...
      print(
        "❌ NotificationManager: Error requesting notification permission: $e",
      );
    } finally {
