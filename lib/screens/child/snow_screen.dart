...
class _SnowScreenState extends State<SnowScreen> {
  ...
  void initState() {
    super.initState();
    SharedPrefsHelper.setHasVisitedSnow(true);
    _loadPlacedItems();
  }
  ...
  Future<void> _loadPlacedItems() async {
    ...
    // 全てのアイテムのパスを結合
    final allItems = [...snowItems, ...snowLivings];
  ...
  double _getItemSize(String itemPath) {
    ...
    if (itemPath.contains('item_kamakura')) return 100.0;
    if (itemPath.contains('item_yuki_ki')) return 100.0;
    if (itemPath.contains('item_yukidaruma')) return 100.0;
    if (itemPath.contains('item')) return 50.0;
    if (itemPath.contains('living_sirokuma')) return 100.0;
    if (itemPath.contains('living_tonakai')) return 80.0;
    if (itemPath.contains('living_simaenaga')) return 40.0;
          ...
          Container(
            decoration: const BoxDecoration(
              image: DecorationImage(
                image: AssetImage('assets/images/snow_background.png'),
                fit: BoxFit.cover,
              ),
            ),
          ),
          ...
          RepaintBoundary(
            ...
            child: Stack(
              children: [
                ...
                ..._equippedSnowItems.map((itemPath) {
                ...
                ..._equippedSnowLivings.map((itemPath) {
                  return DraggableCharacter(
                    id: itemPath,
                    imagePath: itemPath,
                    position:
                ...
                ..._equippedCharacters.map((charPath) {
                  return DraggableCharacter(
                    id: 'snow_$charPath', // IDとして画像パスを使う
                    imagePath: charPath,
                    position:
